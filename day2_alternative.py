...
class Reports:
    ...
    def safe_reports(self):
        safe_reports = [report if report.is_safe(with_dampener=True) else None for report in self.reports]
        return filter(lambda x: x is not None, safe_reports)
...
def check_numbers(report:[int], times=0):
    old_number = report[0]
    for index, number in enumerate(report[1:]):
...
class Report:
    ...
    def __str__(self):
        return ' '.join(str(x) for x in self.report)
...
def main():
    ...
    with open('temp.txt', 'w') as output_file:
        for report in reports.safe_reports():
